...
def entrenar_modelo_global():
    ...
    df = builder.construir_dataset_global()

    if df is None or len(df) < 100:
        print("No existen suficientes datos a nivel global para entrenar el modelo.")
        return None
    ...
    print(f"Modelo Global - MAE: {mae}")

    models_dir = os.path.join(settings.BASE_DIR, 'media', 'models')
    os.makedirs(models_dir, exist_ok=True)
    
    ruta_modelo = os.path.join(models_dir, "global_model.joblib")
    ...
    print(f"Modelo Global guardado en {ruta_modelo}")
    return {"mae": mae, "registros_usados": len(df)}