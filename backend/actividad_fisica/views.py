...
from core.models import ActividadFisica
from .serializers import ActividadFisicaSerializer, ActividadFisicaListSerializer


def get_usuario_id(request):
    """Extrae usuario_id del token de Authorization, o del header X-Usuario-Id como fallback."""
    auth_header = request.headers.get('Authorization', '')
    if auth_header.startswith('Token '):
        token_key = auth_header.split(' ')[1]
        try:
            # NOTA: Ajusta 'Tokens' o 'token' según cómo esté definido en tu app 'core'
            from core.models import Tokens  
            token = Tokens.objects.get(token=token_key)
            return token.usuario_id
        except Exception:
            pass
            
    # Fallback para pruebas con curl o Postman sin token
