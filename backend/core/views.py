...
class GlucosaView(APIView):
    ...
    def get(self, request):
        user_id = request.headers.get('X-User-Id') or request.query_params.get('usuario')
        if not user_id:
            return Response({"error": "Se requiere usuario id"}, status=status.HTTP_400_BAD_REQUEST)
        
        # Obtener historial o ultima
        limit = request.query_params.get('limit')
        qs = Glucosa.objects.filter(usuario_id=user_id).order_by('-fecha', '-hora')
        ...
        serializer = GlucosaSerializer(qs, many=True)
        return Response(serializer.data)
